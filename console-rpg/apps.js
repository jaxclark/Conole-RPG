...
console.log(`Zombies roam the streets hungry for brains, ready to infect what few survivors are left, but they aren't the only danger here. `)
...
function zombieOnlyAttack(randZom) {
    ...
    if(hero.hp > 0 && randZom.hp > 0) {
        ...
        console.log(`The ${randZom.name} defeats you! It feasts on your brians, and what's left of your corpse rises as a zombie, cursed to forever wander and hunger in undeath.`)
        isAlive = false
...
function lookAround() {
    if(walked < 240){
        if(walked >= looked) {
            ...
            let shelterChance = Math.floor(Math.random() * 300)
            let attackChance = 0
